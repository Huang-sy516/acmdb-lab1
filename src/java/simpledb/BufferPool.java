...
public class BufferPool {
    ...
    public  Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        // some code goes here
        // retrival the specific page
        Page page = pagesMap.get(pid);
        // if not found, then load it from disk
        if (page == null) {
            // the case that BufferPOol is full
            if (pagesMap.size() >= numPages) {
                throw new DbException("BufferPool is full...");
            }
            page = loadPageFromDisk(pid);
    ...
    private Page loadPageFromDisk(PageId pid) throws DbException {
        try {
            // read successfully
            DbFile dbFile = Database.getCatalog().getDatabaseFile(pid.getTableId());
            return dbFile.readPage(pid);
        } catch (Exception e) {
            throw new DbException("Error loading page from disk: " + e.getMessage());
        }
    }
